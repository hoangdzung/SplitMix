...
if __name__ == '__main__':
    ...
    # Data
    train_loaders, val_loader, test_loader = fed.get_data()
    ...
    for a_iter in range(start_epoch, args.iters):
        ...
        for client_idx in fed.client_sampler.iter():
            ...
            # print(f' User-{client_idx} Train | Loss: {train_loss:.4f} |'
            #       f' Acc: {train_acc:.4f} | Elapsed: {elapsed:.2f} s')
            # print({
            #     f"{client_idx} train_loss": train_loss,
            #     f"{client_idx} train_acc": train_acc,
            # })

        # Use accumulated model to update server model
        fed.aggregate()

        fed.download(running_model, 0)

        if isinstance(running_model, EnsembleNet):
            running_model.switch_slim_mode(1.0)
            val_mix_model = running_model
        else:
            # FIXME ad-hoc for SlimmableNet
            running_model.switch_slim_mode(1.0)  # full net should load the full net
        ...
        # Test
        if running_model.bn_type.startswith('d'):
            val_loss, val_acc = test_dbn(val_mix_model, val_loader, loss_fun, device,
                                        adversary=adversary, att_BNn=True, detector='gt')
        else:
        ...
        if val_acc > best_val_acc:
            ...
            if running_model.bn_type.startswith('d'):
                _, best_test_acc = test_dbn(val_mix_model, test_loader, loss_fun, device,
                                            adversary=adversary, att_BNn=True, detector='gt')
            else:
    ...
    with open(os.path.join(save_dir, 'log.json'), 'w') as f:
        ...
        #     print({'val_sacc': np.mean(val_acc_list)})
        ...
        #     print({'val_racc': np.mean(val_racc_list)})
